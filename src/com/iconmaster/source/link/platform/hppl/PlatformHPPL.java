...
public class PlatformHPPL extends Platform {
	...
	private String assembleFunction(AssemblyData ad, Function fn) {
	...
	private String assembleField(AssemblyData ad, Field var) {
		StringBuilder sb = new StringBuilder();
	...
	private String assembleCode(AssemblyData ad, ArrayList<Operation> expr) {
	...
	private String assembleExpression(AssemblyData ad, ArrayList<Operation> expr, Operation op) {
		StringBuilder sb = new StringBuilder();
		if (op.op.isMathOp()) {
			sb.append(getInlineString(ad, expr, op.args[1]));
			...
			switch (op.op) {
				...
				case CALL:
					...
					if (fn!=null) {
						if (fn.isLibrary()) {
							String oncomp = fn.compileFunction(ad.pkg,new PlatformContext(expr, op, sb, this, ad));
							if (oncomp!=null) {
								sb.append(oncomp);
								break;
							}
	...
	public void addLocal(AssemblyData ad, ArrayList<Operation> code, Operation thisOp, StringBuilder sb) {
		boolean need = true;
	...
	public String getInlineString(AssemblyData ad, ArrayList<Operation> code, String var) {
	...
	public boolean canRemove(AssemblyData ad, ArrayList<Operation> ops, Operation op, Stack<AssembleVarSpace> vs) {
		if (!op.op.hasLVar()) {
			return false;
		}
...
//		return false;
	...
	public boolean ditchLValue(AssemblyData ad, ArrayList<Operation> ops, Operation op) {
		if (Directives.has(ad.dirs, "!optimize")) {
			return false;
		}
		if (!op.op.hasLVar()) {
